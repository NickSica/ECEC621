...
void tick(Controller *controller)
{
	...
	#ifdef OUTOFORDER
	Node *curr_node = controller->waiting_queue->first;

	while(curr_node != NULL)
	{
	    int target_bank_id = curr_node->bank_id;
	    if ((controller->bank_status)[target_bank_id].next_free <= controller->cur_clk)
	    {
		
		curr_node->begin_exe = controller->cur_clk;
		if (curr_node->req_type == READ)
		{
		    curr_node->end_exe = curr_node->begin_exe + (uint64_t)nclks_read;
		}
		else if (curr_node->req_type == WRITE)
		{
		    curr_node->end_exe = curr_node->begin_exe + (uint64_t)nclks_write;
		}
		// The target bank is no longer free until this request completes.
		(controller->bank_status)[target_bank_id].next_free = curr_node->end_exe;

		migrateToQueue(controller->pending_queue, curr_node);
		deleteNode(controller->waiting_queue, curr_node);
		break;
	    }

	    if (curr_node == controller->waiting_queue->last)
		break;
